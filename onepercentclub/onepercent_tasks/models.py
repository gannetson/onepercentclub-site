...
class SupportedProjectsManager(models.Manager):
    ...
    def by_user(self, user):
        """
        Dummy function that will return None.
        """
        from onepercent_projects.models import OnePercentProject
        projects = OnePercentProject.objects.none()
        return projects


class OnePercentTask(models.Model):
    """ Tasks """
    ...
    def __init__(self, *args, **kwargs):
        super(OnePercentTask, self).__init__(*args, **kwargs)
        self._original_status = self.status

    def get_meta_title(self, **kwargs):
        from onepercent_projects.models import ProjectPlan
        try:
...
class TaskMember(models.Model):
    ...
    task = models.ForeignKey('OnePercentTask')
...
### SIGNALS ###
@receiver(pre_save, weak=False, sender=OnePercentTask, dispatch_uid="log-task-status")
