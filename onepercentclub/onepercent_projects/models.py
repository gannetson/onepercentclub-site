import datetime
from onepercent_tasks.models import OnePercentTask as Task
...
from bluebottle.projects.models import BaseProject, ProjectTheme
...
class OnePercentProject(BaseProject):
    ...
    @property
    def task_count(self):
        return len(self.onepercenttask_set.filter(status=Task.TaskStatuses.open).all())

    @property
    def get_open_tasks(self):
        return self.onepercenttask_set.filter(status=Task.TaskStatuses.open).all()
...
class ProjectPitch(models.Model):
    ...
    project = models.OneToOneField("onepercent_projects.OnePercentProject", verbose_name=_("project"))
